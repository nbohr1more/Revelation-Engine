...
====================
*/
static void ClipOccluders(idVec4 *verts, glIndex_t *indexes, int numIndexes, const idVec3 &projectionOrigin) {
	int					numTris = numIndexes / 3;
	int					i;
	shadowTri_t			*tris = (shadowTri_t *)_alloca8(numTris * sizeof(*tris));
	shadowTri_t			*tri;
...
static void GenerateSilEdges(void) {
	int				i, j;
	unsigned int	*edges = (unsigned *)_alloca8((numOutputTris * 3 + 1) * sizeof(*edges));
	int				numEdges = 0;
	numSilEdges = 0;
	for (i = 0; i < numOutputTris; i++) {
...
======================
*/
static void ProjectUniqued(const idVec3 &projectionOrigin, idPlane projectionPlane) {
...
====================
*/
optimizedShadow_t SuperOptimizeOccluders(idVec4 *verts, glIndex_t *indexes, int numIndexes, idPlane projectionPlane, const idVec3 &projectionOrigin) {
