...
int idSurface::Split(const idPlane &plane, const float epsilon, idSurface **front, idSurface **back, int *frontOnPlaneEdges, int *backOnPlaneEdges) const {
	...
	idSurface 		*surface[2];
	idDrawVert		v;
	dists = (float *)_alloca8(verts.Num() * sizeof(float));
	sides = (byte *)_alloca8(verts.Num() * sizeof(byte));
	...
	*back = surface[1] = new idSurface();
	edgeSplitVertex = (int *)_alloca8(edges.Num() * sizeof(int));
	...
	vertexRemap[0] = (int *)_alloca8(verts.Num() * sizeof(int));
	memset(vertexRemap[0], -1, verts.Num() * sizeof(int));
	vertexRemap[1] = (int *)_alloca8(verts.Num() * sizeof(int));
	memset(vertexRemap[1], -1, verts.Num() * sizeof(int));
	vertexCopyIndex[0] = (int *)_alloca8((numEdgeSplitVertexes + verts.Num()) * sizeof(int));
	vertexCopyIndex[1] = (int *)_alloca8((numEdgeSplitVertexes + verts.Num()) * sizeof(int));
	...
	onPlaneEdges[0] = (int *)_alloca8((maxOnPlaneEdges + 1) * sizeof(int));
	onPlaneEdges[1] = (int *)_alloca8((maxOnPlaneEdges + 1) * sizeof(int));
...
bool idSurface::ClipInPlace(const idPlane &plane, const float epsilon, const bool keepOn) {
	...
	// allocate indexes to construct the triangle indexes for the front and back surface
	vertexRemap = (int *)_alloca8(verts.Num() * sizeof(int));
	memset(vertexRemap, -1, verts.Num() * sizeof(int));
	vertexCopyIndex = (int *)_alloca8((numEdgeSplitVertexes + verts.Num()) * sizeof(int));
	vertexIndexNum[0] = 0;
...
bool idSurface::RayIntersection(const idVec3 &start, const idVec3 &dir, float &scale, bool backFaceCull) const {
	...
	idPluecker rayPl, pl;
	idPlane plane;
	sidedness = (byte *)_alloca8(edges.Num() * sizeof(byte));
