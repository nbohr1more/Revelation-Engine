...
void idSurface_Polytope::FromPlanes(const idPlane *planes, const int numPlanes) {
	...
	idFixedWinding w;
	idDrawVert newVert;
	windingVerts = (int *)_alloca8(MAX_POINTS_ON_WINDING * sizeof(int));
...
int idSurface_Polytope::SplitPolytope(const idPlane &plane, const float epsilon, idSurface_Polytope **front, idSurface_Polytope **back) const {
	...
	int *onPlaneEdges[2];
	onPlaneEdges[0] = (int *)_alloca8(indexes.Num() / 3 * sizeof(int));
	onPlaneEdges[1] = (int *)_alloca8(indexes.Num() / 3 * sizeof(int));
