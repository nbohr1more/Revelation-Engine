...
*/
template< class type >
ID_INLINE idLinkList<type>::idLinkList() {
	owner = NULL;
	head = this;
	next = this;
	prev = this;
...
*/
template< class type >
ID_INLINE int idLinkList<type>::Num(void) const {
...
*/
template< class type >
ID_INLINE void idLinkList<type>::Clear(void) {
	if (head == this) {
		while (next != this) {
			next->Remove();
		}
...
*/
template< class type >
ID_INLINE void idLinkList<type>::Remove(void) {
	prev->next = next;
	next->prev = prev;
	next = this;
	prev = this;
...
*/
template< class type >
ID_INLINE void idLinkList<type>::InsertBefore(idLinkList &node) {
	Remove();
	next = &node;
	prev = node.prev;
	node.prev = this;
...
*/
template< class type >
ID_INLINE void idLinkList<type>::InsertAfter(idLinkList &node) {
	Remove();
	prev = &node;
	next = node.next;
	node.next = this;
...
*/
template< class type >
ID_INLINE type *idLinkList<type>::Prev(void) const {
	if (!prev || (prev == head)) {
		return NULL;
	}
	return prev->owner;
...
*/
template< class type >
ID_INLINE idLinkList<type> *idLinkList<type>::NextNode(void) const {
	if (next == head) {
		return NULL;
	}
	return next;
...
*/
template< class type >
ID_INLINE idLinkList<type> *idLinkList<type>::PrevNode(void) const {
	if (prev == head) {
		return NULL;
	}
	return prev;
...
*/
template< class type >
ID_INLINE type *idLinkList<type>::Owner(void) const {
	return owner;
}
...
*/
template< class type >
ID_INLINE void idLinkList<type>::SetOwner(type *object) {
	owner = object;
}
