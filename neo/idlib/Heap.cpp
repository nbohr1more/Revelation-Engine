...
void *idHeap::Allocate16(const dword bytes) {
	byte *ptr, *alignedPtr;
	ptr = (byte *)GC_MALLOC_WRAPPER(bytes + 16 + 4); // memory leak here according to GC
	if (!ptr) {
		if (defragBlock) {
...
dword idHeap::Msize(void *p) {
	...
	switch (((byte *)(p))[-1]) {
		...
		default: {
			idLib::common->Warning("idHeap::Msize: invalid memory block (%s)", idLib::sys->GetCallStackCurStr(4));
			return 0;
		}
	}
#endif
...
void *Mem_Alloc(const int size) {
	...
	if (!mem_heap) {
		...
		*((int *)0x0) = 1;
#endif
		// this is a very large alloc at times,
		// so use this macro to avoid GC crashing
