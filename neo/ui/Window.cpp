...
bool idWindow::Parse(idParser *src, bool rebuild) {
	...
	while (token != "}") {
		...
		//  added new onEvent
			...
			src->SetMarker();
			// how about we add the var to the desktop instead of this window so it won't get deleted
			// when this window is destroyed which even happens during parsing with simple windows ?
			...
			// FIXME unused but cannot be removed or is it ???.
			parsed = true;
		}
			...
			// callback on parsing
			parsed = ParseRegEntry(token, src);
		}
		if (!src->ReadToken(&token)) {
			src->Error("Unexpected end of file");
			ret = false;
	...
	if (parsed) {
		common->DPrintf("Parsed all entries\n");
	}
	if (ret) {
		EvalRegs(-1, true);
	}
