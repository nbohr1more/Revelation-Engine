...
===========
*/
void idVertexCache::MapBufferRange(vertCache_t *block, void *data, int size) {
	...
	if (glConfig.ARBMapBufferRangeAvailable && r_useArbBufferRange.GetBool()) {
		...
		GLvoid      *ptr = glMapBufferRange(GL_ARRAY_BUFFER, static_cast<GLintptr>(block->offset), static_cast<GLsizeiptr>(size), access);
		...
		if (ptr) {
			...
			glBufferSubData(GL_ARRAY_BUFFER, static_cast<GLintptrARB>(block->offset), static_cast<GLsizeiptr>(size), data);
		}
	}
	else {
