...
idCVar r_usePreciseTriangleInteractions("r_usePreciseTriangleInteractions", "1", CVAR_RENDERER | CVAR_BOOL, "1 = do winding clipping to determine if each ambiguous tri should be lit");
...
void R_InitOpenGL(void) {
	...
	cmdSystem->AddCommand("reloadARBprograms", R_Backend_ReloadPrograms_f, CMD_FL_RENDERER, "reloads ARB programs");
	R_Backend_ReloadPrograms_f(idCmdArgs());
...
void R_ReportSurfaceAreas_f(const idCmdArgs &args) {
	...
	list = (idMaterial **)_alloca8(count * sizeof(*list));
	for (i = 0; i < count; i++) {
...
void R_MakeAmbientMap_f(const idCmdArgs &args) {
	...
	int	samples = 1000;
	byte	*outBuffer = (byte *)_alloca8(outSize * outSize * 4);
