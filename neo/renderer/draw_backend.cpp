...
void RB_Backend_CreateDrawInteractions(const drawSurf_t *surf) {
	...
	for (/**/; surf; surf = surf->nextOnLight) {
		...
		// this may cause RB_Backend_DrawInteraction to be executed multiple
...
static progDef_t progs[MAX_GLPROGS] = {
		...
		// changed the unused glasswarp calls here to the megatexture ones that might actually be used in mods.
...
/*
=================
R_Backend_LoadPrograms
=================
*/
void R_Backend_LoadPrograms(int progIndex) {
	int		ofs;
	int		err;
	...
	buffer = (char *)_alloca8(strlen(fileBuffer) + 1);
...
int R_FindARBProgram(GLenum target, const char *program) {
	...
	progs[i].ident = (program_t)0;	// will be gen'd by R_Backend_LoadPrograms
	progs[i].target = target;
	strncpy(progs[i].name, program, sizeof(progs[i].name) - 1);
	R_Backend_LoadPrograms(i);
	return progs[i].ident;
}

/*
==================
R_Backend_ReloadPrograms_f
==================
*/
void R_Backend_ReloadPrograms_f(const idCmdArgs &args) {
	int	i;
