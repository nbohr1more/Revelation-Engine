...
==================
*/
static void R_SkyboxTexGen(drawSurf_t *surf, const idVec3 &viewOrg) {
	int					i;
...
==================
*/
static void R_WobbleskyTexGen(drawSurf_t *surf, const idVec3 &viewOrg) {
	int			i;
	idVec3		localViewOrigin;
	...
	transform[0 * 4 + 3] = transform[1 * 4 + 3] = transform[2 * 4 + 3] = 0.0f;
...
static bool R_TestPointInViewLight(const idVec3 &org, idRenderLightLocal *light) {
	for (int i = 0; i < 6; i++) {
...
/*
===================
R_TestPointInFrustum

Assumes positive sides face outward
===================
*/
static const float INSIDE_LIGHT_POINT_SLOP = 0.0f;
static bool R_TestPointInFrustum(const idVec3 &point, idPlane *planes, int numPlanes) {
	for (int i = 0; i < numPlanes; i++) {
		float d = planes[i].Distance(point);
		if (d > INSIDE_LIGHT_POINT_SLOP) {
			return false;
		}
	}
	return true;
...
viewLight_t *R_SetLightDefViewLight(idRenderLightLocal *light) {
	...
	vLight->viewSeesGlobalLightOrigin = R_TestPointInFrustum(light->globalLightOrigin, tr.viewDef->frustum, 4);
...
==================
*/
static viewEntity_t* R_SortViewEntities(viewEntity_t* vEntities) {
...
==================
*/
static idScreenRect R_CalcEntityScissorRectangle(viewEntity_t *vEntity) {
	idBounds			bounds;
...
static const int LIGHT_INTERACTIONS = 4096;
void R_AddModelSurfaces(void) {
	viewEntity_t		*vEntity;
	idRenderModel		*model;
	idInteraction		*inter, *next;
	idInteraction		*interactions[LIGHT_INTERACTIONS];
	idRenderModel		*createInteractionModel[LIGHT_INTERACTIONS];
	idRenderModel		*interactionModelPtr[LIGHT_INTERACTIONS];
	idScreenRect		shadowScissor[LIGHT_INTERACTIONS];
	bool				interactionPhase2[LIGHT_INTERACTIONS];
	int					createInteractionId[LIGHT_INTERACTIONS];
	int					nInteractions = 0;
		...
		if (tr.viewDef->isXraySubview) {
			if (vEntity->entityDef->parms.xrayIndex == 2) {
				for (inter = vEntity->entityDef->firstInteraction; inter != NULL && !inter->IsEmpty(); inter = next) {
					...
					assert(nInteractions <= LIGHT_INTERACTIONS);
				}
			}
		}
		else {
...
void R_RemoveUnecessaryViewLights(void) {
	...
	sortLight_t *sortLights = (sortLight_t *)_alloca8(sizeof(sortLight_t) * numViewLights);
