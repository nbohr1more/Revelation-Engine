...
template< class type >
ID_STATIC_INLINE idDecl *idDeclAllocator(void) {
	return new type;
}
...
ID_STATIC_INLINE void idListDecls_f(const idCmdArgs &args) {
	declManager->ListType(args, type);
}

template< declType_t type >
ID_STATIC_INLINE void idPrintDecls_f(const idCmdArgs &args) {
