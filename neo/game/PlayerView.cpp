...
idPlayerView::idPlayerView() {
	...
	currentDepthMaterial	= declManager->FindMaterial( "_currentDepth" );
...
void idPlayerView::PostFX_LensFlare() {
	...
	if( VdotS[ 0 ] > 0 ) {
		...
		if( trace.c.material->NoFragment() || trace.fraction == 1.0f ) {  // Trace succeeded, or it hit a skybox
			...
			renderSystem->SetColor4( strength * 0.1f, strength * 0.3f, strength * 1.0f, 1.0f );
			...
			renderSystem->SetColor4( strength * 0.1f, strength * 0.3f, strength * 1.0f, 2.0f );
			renderSystem->DrawStretchPic( uv.x, uv.y, 32.0f, 32.0f, 0.0f, 1.0f, 1.0f, 0.0f, lensFlareMaterial );
			...
			renderSystem->SetColor4( strength * 0.1f, strength * 0.3f, strength * 1.0f, 3.0f );
			renderSystem->DrawStretchPic( uv.x, uv.y, 64.0f, 64.0f, 0.0f, 1.0f, 1.0f, 0.0f, lensFlareMaterial );
			...
			renderSystem->SetColor4( strength * 0.1f, strength * 0.3f, strength * 1.0f, 4.0f );
			renderSystem->DrawStretchPic( uv.x, uv.y, 32.0f, 32.0f, 0.0f, 1.0f, 1.0f, 0.0f, lensFlareMaterial );
			...
			renderSystem->SetColor4( strength * 0.1f, strength * 0.3f, strength * 1.0f, 5.0f );
